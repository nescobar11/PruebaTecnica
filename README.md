...
## Prueba Frontend
 1. Angular 17 standalone.
 2. Angular Material.
 3. Tabla responsive.
 4. Paginación.
 5. Filtro por ciudad.
 6. Loading/error states.
 7. Tipado con interfaces.
 8. Separación de services y models.
 9. Tests unitarios básicos.
 10. README y Dockerfile.
## Prueba Backend
...
`Database/01_CrearBaseDeDatosYTabla.sql` — crea la base de datos `DBClientes`
y la tabla `Clientes`.
`Database/02_StoredProcedure.sql` — crea `sp_ObtenerCliente`, que recibe
`@NumeroIdentificacion` y devuelve los datos del cliente.
`Database/03_DatosDePrueba.sql` (opcional) — inserta 3 clientes de ejemplo
...
Abrir `ClientesApi.csproj` (o crear una solución `.sln` que lo contenga).
Restaurar paquetes NuGet (se restauran automáticamente al abrir).
Presionar `F5` / `Iniciar` (perfil IIS Express o Kestrel).
El navegador abrirá Swagger automáticamente en la raíz `/`.
